...
| [`add-integration-test-runner.md`](./add-integration-test-runner.md) | One-command `hack/run-integration-tests.sh` for the `-tags integration` suite (auto-detects macOS/Linux), plus the missing `hack/edit-probe` image so `KRAYT_IMAGE`-dependent tests need no live LLM credential, plus a path-filtered Linux CI job. | ✅ Done — `hack/run-integration-tests.sh` (OS-detecting, `doctor` preflight, base-image + probe-ref resolution, Linux compile→setcap→run), `hack/edit-probe` wired into `probe-images.yml`, path-filtered `integration-linux` job in `ci.yml`. Real-VM verification (macOS suite, `edit-probe` build/push, first CI run) handed off in `HUMAN_TODO.md` — no `/dev/kvm` in the sandbox |
