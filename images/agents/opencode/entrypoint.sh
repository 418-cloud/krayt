...
# placeholder value, which only has to satisfy opencode's own "a credential is configured"
...
if [ -n "${KRAYT_CA_CERT:-}" ] && [ -f "${KRAYT_CA_CERT}" ]; then
  ...
  echo "[opencode] trusting krayt's ephemeral MITM CA (network.mitm enabled)"
fi
...
opencode run --model "$model" --auto "$(cat "$TASK_FILE")" | tee "$OUTPUT_DIR/report.md"
echo "[opencode] done"
