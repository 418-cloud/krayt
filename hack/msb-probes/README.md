...
|---|---|---|
| `p1-vsock-nonroot.sh` | Can a **non-root** guest process (`agent`, uid 1000) open `AF_VSOCK` to reach the host? Also doubles as "does a krayt agent image run unmodified under msb". | **Yes** — decides `dial-ask-channel-over-vsock.md`'s whole design (a direct `krayt-ask` dial vs. a root-owned in-guest forwarder). |
