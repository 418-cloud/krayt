...
**Open:** the `krayt-agent-claude-code` and (new) `krayt-agent-gemini-cli` published images each
need a real CI run + a real live onboarding run — see the `[tooling]` entries below. The
gemini-cli image also needs its `node:24-bookworm-slim` base digest pinned (currently tag-only —
this sandbox's egress proxy has no route to a Docker registry) and a `hadolint` pass (binary
unreachable here too). All non-blocking (nothing downstream depends on them yet; the opencode
agent-image task builds on the *code* landing here, not on these verifications).
