...
// ca_cert is the PEM-encoded PUBLIC certificate of this run's ephemeral MITM CA (§6.6,
// add-tls-mitm-credential-injection.md) — empty when network.mitm is false. Never the private
// key: the guest only needs to trust connections the host proxy terminates, not sign anything.
...
func (x *NetworkPolicy) GetCaCert() []byte {
	if x != nil {
		return x.CaCert
	}
	return nil
...
const file_krayt_proto_rawDesc = "" +
	...
	"\x05allow\x18\x02 \x03(\tR\x05allow\x12\x17\n" +
	"\aca_cert\x18\x03 \x01(\fR\x06caCert\")\n" +
	"\x04Mode\x12\r\n" +
