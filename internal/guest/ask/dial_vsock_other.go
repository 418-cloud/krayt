...
// logic above on darwin. It still wraps ErrVsockUnsupported, not a bare error, so that if it is
// ever reached (e.g. cmd/krayt-ask built and run directly on a non-linux host), a well-formed
// vsock:// value fails loudly as a usage error instead of masquerading as "no human available".
