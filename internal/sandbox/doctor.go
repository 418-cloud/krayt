...
// msb installed today, a hard failure here would newly fail `krayt doctor` — and with it
// hack/run-integration-tests.sh's preflight — for a dependency nothing yet needs. That is exactly
// the "no existing behaviour changes" this task promises. run-tasks-on-microsandbox.md, which
// wires krayt run through msb and deletes the vfkit/firecracker checks, is the point these stop
// being optional.
...
func versionCheck(ctx context.Context, c *Client) CheckResult {
	...
	v, err := c.Version(ctx)
	if err != nil {
		return CheckResult{Name: name, Optional: true, Detail: "msb --version failed: " + err.Error()}
	}
	if v.Less(MinVersion) {
		return CheckResult{Name: name, Optional: true, Detail: fmt.Sprintf(
			"msb %s is below krayt's minimum supported version %s (raw: %q) — upgrade msb", v, MinVersion, v.Raw)}
	}
	return CheckResult{Name: name, OK: true, Optional: true, Detail: v.String()}
}
...
func contextCheck(ctx context.Context, c *Client) CheckResult {
	...
	if err != nil {
		return CheckResult{Name: name, Optional: true, Detail: "msb context --format json failed: " + err.Error()}
	}
	if !info.IsLocal() {
	...
	return CheckResult{Name: name, OK: true, Optional: true, Detail: "backend=" + info.Backend}
}
...
func passthroughDoctorCheck(ctx context.Context, c *Client) CheckResult {
	...
	return CheckResult{Name: name, OK: true, Optional: true, Detail: detail}
}
