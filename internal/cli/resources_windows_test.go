...
// These are smoke tests, not fixed-value tests: this machine has a real GlobalMemoryStatusEx and
// a real filesystem, so we can only assert the measurement functions return plausible values, not
// exact ones (mirrors resources_darwin_test.go).
