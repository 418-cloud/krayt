...
func checkHostResources(freeMemMiB, freeDiskGiB, wantMemMiB, wantDiskGiB uint64) error {
	// Subtraction (guarded by the > check) avoids uint64 overflow when want* is very large.
	if wantMemMiB > freeMemMiB || freeMemMiB-wantMemMiB < memMarginMiB {
		return fmt.Errorf(
	...
	if wantDiskGiB > freeDiskGiB || freeDiskGiB-wantDiskGiB < diskMarginGiB {
		return fmt.Errorf(
