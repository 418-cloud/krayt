...
//
// Every guest step here runs as sandboxAgentUser, never root (decision 4): file ownership then
...
func applyConfigSeeds(ctx context.Context, sb *sandbox.Client, name string, seeds []task.ConfigSeed, warnOut io.Writer) {
	for _, seed := range seeds {
		if err := applyConfigSeed(ctx, sb, name, seed); err != nil {
...
func applyConfigSeed(ctx context.Context, sb *sandbox.Client, name string, seed task.ConfigSeed) error {
	...
	if err := writeSeedFile(ctx, sb, name, guestPath, merged); err != nil {
...
// readSeedFile execs `cat guestPath` as the agent user. A missing (or otherwise unreadable) file
...
func readSeedFile(ctx context.Context, sb *sandbox.Client, name, guestPath string) ([]byte, error) {
...
// writeSeedFile execs a small shell script, as the agent user, that mkdir -p's guestPath's parent,
...
func writeSeedFile(ctx context.Context, sb *sandbox.Client, name, guestPath string, content []byte) error {
